...
<head>
  ...
  <style>
    :root{--bg:#060914;--panel:rgba(15,23,42,.88);--card:rgba(17,24,39,.9);--line:rgba(148,163,184,.2);--text:#e5e7eb;--muted:#94a3b8;--ok:#22c55e;--warn:#f59e0b;--bad:#fb7185;--blue:#38bdf8}*{box-sizing:border-box}body{margin:0;min-height:100vh;font-family:Arial,Helvetica,sans-serif;color:var(--text);background:radial-gradient(circle at 15% 0%,rgba(56,189,248,.16),transparent 28%),radial-gradient(circle at 85% 10%,rgba(34,197,94,.13),transparent 26%),var(--bg);padding:22px}.shell{max-width:1180px;margin:auto}.hero,.grid{display:grid;gap:16px}.hero{grid-template-columns:1fr}.grid{grid-template-columns:1fr}@media(min-width:850px){.hero{grid-template-columns:1.4fr .8fr}.grid.cols4{grid-template-columns:repeat(4,1fr)}.grid.cols2{grid-template-columns:repeat(2,1fr)}}.panel,.card{border:1px solid var(--line);background:var(--panel);border-radius:24px;box-shadow:0 24px 80px rgba(0,0,0,.32)}.panel{padding:24px}.card{padding:18px}h1{font-size:clamp(30px,5vw,52px);margin:8px 0;letter-spacing:-1px}h2{margin:0 0 12px;font-size:18px}p{color:var(--muted);line-height:1.55}.pill{display:inline-flex;align-items:center;gap:8px;border:1px solid var(--line);border-radius:999px;padding:8px 12px;background:rgba(2,6,23,.48);font-weight:700}.dot{width:10px;height:10px;border-radius:999px;background:var(--warn);box-shadow:0 0 20px currentColor}.dot.ok{background:var(--ok);color:var(--ok)}.dot.bad{background:var(--bad);color:var(--bad)}.metric{border:1px solid var(--line);background:var(--card);border-radius:18px;padding:16px}.label{color:var(--muted);font-size:12px;text-transform:uppercase;letter-spacing:.08em}.value{font-size:24px;font-weight:900;margin-top:8px;word-break:break-word}.small{color:var(--muted);font-size:13px;margin-top:6px;word-break:break-word}input,button{width:100%;padding:13px 14px;border-radius:14px;border:1px solid var(--line);background:rgba(2,6,23,.65);color:var(--text);font-size:15px;margin-top:8px}button{cursor:pointer;background:linear-gradient(135deg,var(--blue),var(--ok));color:#020617;font-weight:900;border:0}.secondary{background:rgba(148,163,184,.12);color:var(--text);border:1px solid var(--line)}.qrbox{min-height:300px;display:grid;place-items:center;border:1px dashed rgba(148,163,184,.28);border-radius:22px;background:rgba(2,6,23,.36);text-align:center;padding:18px}.qrbox img{width:300px;max-width:100%;background:white;padding:12px;border-radius:18px}.section{margin-top:16px}.kv{display:grid;grid-template-columns:1fr;gap:10px}@media(min-width:760px){.kv{grid-template-columns:210px 1fr}}.k{color:var(--muted)}.v{word-break:break-word}.badge{display:inline-flex;margin:3px 4px 3px 0;padding:7px 10px;border-radius:999px;background:rgba(148,163,184,.12);border:1px solid var(--line);font-size:13px}.badge.ok{background:rgba(34,197,94,.12);border-color:rgba(34,197,94,.35)}.badge.bad{background:rgba(251,113,133,.12);border-color:rgba(251,113,133,.35)}pre{white-space:pre-wrap;word-break:break-word;background:rgba(2,6,23,.58);border:1px solid var(--line);padding:14px;border-radius:16px;color:#fecaca;max-height:280px;overflow:auto}a{color:#7dd3fc;text-decoration:none}.links{display:flex;gap:10px;flex-wrap:wrap}
  </style>
</head>
<body>
  <main class="shell">
    <section class="hero">
      <div class="panel">
        ...
        <h1>⚔️ ${botName}</h1>
        <p>Painel profissional de conexão do bot com atualização automática, diagnóstico, ambiente, erros, QR Code e testes rápidos.</p>
      ...
      <div class="panel">
        <h2>🔐 QR Code</h2>
        <p>Digite a senha QR_PASSWORD para carregar o QR sem sair do painel.</p>
